#
#  A wrapper around redis that namespaces keys
#
...
class RigidojRedis
  ...
  def initialize
    @config = RigidojRedis.config
    @redis = RigidojRedis.raw_connection(@config)
  end
  ...
  [:append, :blpop, :brpop, :brpoplpush, :decr, :decrby, :exists, :expire, :expireat, :get, :getbit, :getrange, :getset,
   ...
   :zremrangebyscore, :zrevrange, :zrevrangebyscore, :zrevrank, :zrangebyscore].each do |m|
    define_method m do |*args|
      args[0] = "#{RigidojRedis.namespace}:#{args[0]}"
      @redis.send(m, *args)
    end
  end

  def del(k)
    k = "#{RigidojRedis.namespace}:#{k}"
    @redis.del k
  end

  def keys(pattern=nil)
    len = RigidojRedis.namespace.length + 1
    @redis.keys("#{RigidojRedis.namespace}:#{pattern || '*'}").map { |k| k[len..-1] }
  end
